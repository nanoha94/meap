...
class RecipeStoreRequest extends BaseApiRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'ingredientCategories.*.isDefault' => 'boolean|nullable',
    ...
    public function messages()
    {
        return [
            ...
            'ingredientCategories.*.name.max' => __('validation.max.string', ['attribute' => 'ingredientCategories.*.name', 'max' => ValidationLimits::STRING_MAX]),
            'ingredientCategories.*.name.required' => __('validation.required', ['attribute' => 'ingredientCategories.*.name']),
            'ingredientCategories.*.isDefault.boolean' => __('validation.boolean', ['attribute' => 'ingredientCategories.*.isDefault']),
            ...
            'ingredients.*.name.max' => __('validation.max.string', ['attribute' => 'ingredients.*.name', 'max' => ValidationLimits::STRING_MAX]),
            ...
            'ingredients.*.categoryName.max' => __('validation.max.string', ['attribute' => 'ingredients.*.categoryName', 'max' => ValidationLimits::STRING_MAX]),
            ...
            'steps.*.instruction.max' => __('validation.max.string', ['attribute' => 'steps.*.instruction', 'max' => ValidationLimits::STRING_MAX]),
            ...
            'memo.max' => __('validation.max.string', ['attribute' => 'memo', 'max' => ValidationLimits::STRING_MAX]),
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($this->has('ingredientCategories') && is_array($ingredientCategories)) {
                $seenCategoryNames = [];
                $seenCategoryIds = [];
                ...
                $defaultCount = collect($ingredientCategories)
                    ->filter(fn ($category) => ! empty($category['isDefault']))
                    ->count();

                if ($defaultCount !== 1) {
                    $validator->errors()->add(
                        'ingredientCategories',
                        __('validation.exactly_one_default_required', [
                            'attribute' => 'ingredientCategories',
                            'field' => 'isDefault',
                            'value' => 'true',
                        ])
                    );
