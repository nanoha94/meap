...
test('Remember Me機能でログインできる', function () {
    $user = User::factory()->create();
...
test('認証情報が不足している場合はログインできない', function () {
    $response = $this->post('/login', []);
...
test('無効なメール形式ではログインできない', function () {
    $response = $this->post('/login', [
        'email' => 'invalid-email',
        'password' => 'password',
    ]);
...
test('未認証ユーザーはログアウトできない', function () {
    $response = $this->post('/logout');

    $response->assertStatus(302); // リダイレクト
});

test('ログイン試行回数の制限が適用される', function () {
    $user = User::factory()->create();
...
test('ログイン成功時にレート制限がクリアされる', function () {
    $user = User::factory()->create();
...
test('ログイン成功時にセッションが再生成される', function () {
    $user = User::factory()->create();
...
test('ログアウト時にセッションが無効化される', function () {
    $user = User::factory()->create();
