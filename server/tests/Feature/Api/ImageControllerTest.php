...
test('3-1-6: 【一括アップロード】 未認証ユーザー', function () {
    $file = UploadedFile::fake()->image('test.jpg', 100, 100);
...
test('3-1-7: 【一括アップロード】 グループが存在しない', function () {
...
test('3-1-8: 【一括アップロード】 ImageService 例外（データベースエラー）', function () {
...
test('3-1-9: 【一括アップロード】 ImageService 例外', function () {
    // ImageServiceのメソッドで例外を発生させる
    $this->mock(ImageService::class, function ($mock) {
        $mock->shouldReceive('getValidImageFiles')
...
test('3-1-10: 【一括アップロード】 ファイルアップロード失敗', function () {
    // ストレージの書き込みを失敗させる
...
test('3-1-17: 【一括削除】 正常な画像削除（1 枚）', function () {
    // 画像をアップロードAPIで作成
    $file = UploadedFile::fake()->image('test.jpg', 100, 100);
...
test('3-1-18: 【一括削除】 複数画像の一括削除', function () {
    // 複数の画像をアップロードAPIで作成
    $files = [
        UploadedFile::fake()->image('test1.jpg', 100, 100),
        UploadedFile::fake()->image('test2.jpg', 100, 100),
...
test('3-1-19: 【一括削除】 削除成功メッセージの確認', function () {
    // 画像をアップロードAPIで作成
    $file = UploadedFile::fake()->image('test.jpg', 100, 100);
    $uploadResponse = $this->actingAs($this->user)->post('/images/upload-bulk', [
        'images' => [$file]
...
test('3-1-20: 【一括削除】 存在しない画像 ID の削除', function () {
    $response = $this->actingAs($this->user)->delete('/images/bulk', [
...
test('3-1-21: 【一括削除】 指定した related_id との紐づけのみを解除', function () {
    // 画像をアップロードAPIで作成
    $file = UploadedFile::fake()->image('test.jpg', 100, 100);
    $uploadResponse = $this->actingAs($this->user)->post('/images/upload-bulk', [
        'images' => [$file]
...
test('3-1-22: 【一括削除】 紐づけ解除のみ（他の紐づけなしでも物理削除は行わない）', function () {
    // 画像をアップロードAPIで作成
    $file = UploadedFile::fake()->image('test.jpg', 100, 100);
    $uploadResponse = $this->actingAs($this->user)->post('/images/upload-bulk', [
        'images' => [$file]
...
test('3-1-23: 【一括削除】 複数の画像で紐づけ解除のみ', function () {
    // 2つの画像をアップロードAPIで作成
    $files = [
        UploadedFile::fake()->image('test1.jpg', 100, 100),
        UploadedFile::fake()->image('test2.jpg', 100, 100)
...
test('3-1-24: 【一括削除】 未認証ユーザー', function () {
...
test('3-1-25: 【一括削除】 グループが存在しない', function () {
    $user = User::factory()->create([
        'email_verified_at' => now()
    ]);
    // グループに所属させない
...
test('3-1-26: 【一括削除】 ImageService 例外（データベースエラー）', function () {
    // 画像をアップロードAPIで作成
    $file = UploadedFile::fake()->image('test.jpg', 100, 100);
    $uploadResponse = $this->actingAs($this->user)->post('/images/upload-bulk', [
        'images' => [$file]
...
test('3-1-27: 【一括削除】 ImageService 例外', function () {
    // 画像をアップロードAPIで作成
    $file = UploadedFile::fake()->image('test.jpg', 100, 100);
    $uploadResponse = $this->actingAs($this->user)->post('/images/upload-bulk', [
        'images' => [$file]
