...
beforeEach(function () {
    ...
    $this->testData = new class {
        ...
        public function createMealTypeViaApi($testInstance, $user, $name = '朝食', $colorId = null, $order = 0)
        {
            if (!$colorId) {
            ...
            $response = $testInstance->actingAs($user)->post('/meal-types', $requestData);
        ...
        public function createCourseTypeViaApi($user, $name = '主菜', $order = 0)
        {
            // CourseTypeはMasterControllerで管理されているため、直接作成
            ...
            return [
                ...
                'order' => $courseType->order
            ];
        }

        public function createMealPlanViaApi($testInstance, $user, $mealTypeId, $recipeId, $courseTypeId, $date = '2024-01-15')
        {
            $requestData = [
                'date' => $date,
        ...
        public function createMealPlan($groupId, $mealTypeId, $date = '2024-01-15')
        {
...
test('3-6-1: 【一覧取得】 正常な献立一覧取得', function () {
    ...
    // エンドポイントを使用してデータを作成
    $mealType = $this->testData->createMealTypeViaApi($this, $user);
    $recipe = $this->testData->createRecipeViaApi($this, $user);
    $courseType = $this->testData->createCourseTypeViaApi($user);

    // エンドポイントを使用して献立を作成
    $mealPlanData = $this->testData->createMealPlanViaApi($this, $user, $mealType['id'], $recipe['id'], $courseType['id']);
...
test('3-6-2: 【一覧取得】 献立データの日付別グループ化確認', function () {
    ...
    $this->testData->createMealPlanViaApi($this, $user, $mealType['id'], $recipe['id'], $courseType['id'], '2024-01-15');
    $this->testData->createMealPlanViaApi($this, $user, $mealType['id'], $recipe['id'], $courseType['id'], '2024-01-16');
    $this->testData->createMealPlanViaApi($this, $user, $mealType['id'], $recipe['id'], $courseType['id'], '2024-01-15');
...
test('3-6-3: 【一覧取得】 レスポンス形式確認', function () {
    ...
    // エンドポイントを使用して献立を作成
    $this->testData->createMealPlanViaApi($this, $user, $mealType['id'], $recipe['id'], $courseType['id']);
...
test('3-6-10: 【新規作成】 未認証ユーザー', function () {
    $requestData = [
        'date' => '2024-01-15',
        'mealTypeId' => \Illuminate\Support\Str::uuid(),
        'menu' => [
            [
                'recipeIds' => [\Illuminate\Support\Str::uuid()],
                'courseTypeId' => \Illuminate\Support\Str::uuid()
            ]
        ]
    ];
...
test('3-6-16: 【新規作成】 バリデーションエラー（mealTypeId 必須）', function () {
    $user = $this->testData->createUserWithGroup();
    $recipe = $this->testData->createRecipeViaApi($this, $user);
    $courseType = $this->testData->createCourseTypeViaApi($user);
    ...
    $response->assertStatus(422);
    $response->assertJsonValidationErrors(['mealTypeId']);
    ...
    $responseData = $response->json();
    $this->assertContains('mealTypeIdは必ず指定してください。', $responseData['errors']['mealTypeId']);
});

test('3-6-17: 【新規作成】 バリデーションエラー（mealTypeId 形式）', function () {
    $user = $this->testData->createUserWithGroup();
    $recipe = $this->testData->createRecipeViaApi($this, $user);
    $courseType = $this->testData->createCourseTypeViaApi($user);
    ...
    $requestData = [
        'date' => '2024-01-15',
        'mealTypeId' => 'invalid-uuid-format', // 無効なUUID形式
        'menu' => [
            [
                'recipeIds' => [$recipe['id']],
                'courseTypeId' => $courseType['id']
    ...
    $responseData = $response->json();
    $this->assertContains('mealTypeIdに有効なUUIDを指定してください。', $responseData['errors']['mealTypeId']);
});

test('3-6-18: 【新規作成】 バリデーションエラー（mealTypeId 存在チェック）', function () {
    $user = $this->testData->createUserWithGroup();
    $recipe = $this->testData->createRecipeViaApi($this, $user);
    $courseType = $this->testData->createCourseTypeViaApi($user);

    // 有効なUUID形式だが存在しないmealTypeIdを使用
    $nonExistentMealTypeId = '12345678-1234-1234-1234-123456789012';

    $requestData = [
        'date' => '2024-01-15',
        'mealTypeId' => $nonExistentMealTypeId, // 存在しないmealTypeId
        'menu' => [
            [
                'recipeIds' => [$recipe['id']],
                'courseTypeId' => $courseType['id']
...
test('3-6-28: 【新規作成】 バリデーションエラー（courseTypeId 必須）', function () {
    $user = $this->testData->createUserWithGroup();
    $mealType = $this->testData->createMealTypeViaApi($this, $user);
    $recipe = $this->testData->createRecipeViaApi($this, $user);
    ...
    $response->assertStatus(422);
    $response->assertJsonValidationErrors(['menu.0.courseTypeId']);
    ...
    $responseData = $response->json();
    $this->assertContains('menu.*.courseTypeIdは必ず指定してください。', $responseData['errors']['menu.0.courseTypeId']);
});

test('3-6-29: 【新規作成】 バリデーションエラー（courseTypeId 形式）', function () {
    $user = $this->testData->createUserWithGroup();
    $mealType = $this->testData->createMealTypeViaApi($this, $user);
    $recipe = $this->testData->createRecipeViaApi($this, $user);
    ...
    $requestData = [
        ...
        'menu' => [
            [
                'recipeIds' => [$recipe['id']],
                'courseTypeId' => 'invalid-uuid-format' // 無効なUUID形式
            ]
        ]
    ];
    ...
    $responseData = $response->json();
    $this->assertContains('menu.*.courseTypeIdに有効なUUIDを指定してください。', $responseData['errors']['menu.0.courseTypeId']);
});

test('3-6-30: 【新規作成】 バリデーションエラー（courseTypeId 存在チェック）', function () {
    $user = $this->testData->createUserWithGroup();
    $mealType = $this->testData->createMealTypeViaApi($this, $user);
    $recipe = $this->testData->createRecipeViaApi($this, $user);

    // 有効なUUID形式だが存在しないcourseTypeIdを使用
    $nonExistentCourseTypeId = '12345678-1234-1234-1234-123456789012';
    ...
    $requestData = [
        ...
        'menu' => [
            [
                'recipeIds' => [$recipe['id']],
                'courseTypeId' => $nonExistentCourseTypeId // 存在しないcourseTypeId
            ]
        ]
    ];
    ...
    $response->assertJson([
        'success' => false,
        'message' => '指定されたapi.attributes.course_typeが見つかりませんでした。'
    ]);
});
...
test('3-6-34: 【詳細取得】 データベース接続エラー', function () {
    ...
    // GroupモデルのmealPlansリレーションをモック
    $groupMock = \Mockery::mock($user->group);
    $groupMock->shouldReceive('mealPlans')
...
test('3-6-36: 【詳細取得】 他グループの献立詳細取得', function () {
    ...
    $otherMealPlan = $this->testData->createMealPlan($otherGroup->id, $mealType['id']);
...
test('3-6-37: 【更新】 正常な献立更新', function () {
    ...
    $mealPlan = $this->testData->createMealPlan($this->testData->defaultGroup->id, $mealType['id']);
    $mealPlan->recipes()->attach($recipe['id'], ['course_type_id' => $courseType['id']]);
...
test('3-6-38: 【更新】 献立の料理更新', function () {
    ...
    $mealPlan = $this->testData->createMealPlan($this->testData->defaultGroup->id, $mealType['id']);
    $mealPlan->recipes()->attach($recipe1['id'], ['course_type_id' => $courseType['id']]);
...
test('3-6-42: 【更新】 データベース接続エラー', function () {
    ...
    $mealPlan = $this->testData->createMealPlanViaApi($this, $user, $mealType['id'], $recipe['id'], $courseType['id']);
...
test('3-6-47: 【更新】 バリデーションエラー（mealTypeId 必須）', function () {
    $user = $this->testData->createUserWithGroup();
    $recipe = $this->testData->createRecipeViaApi($this, $user);
    $courseType = $this->testData->createCourseTypeViaApi($user);

    $mealPlan = $this->testData->createMealPlan($this->testData->defaultGroup->id, $this->testData->createMealTypeViaApi($this, $user)['id']);
...
test('3-6-48: 【更新】 バリデーションエラー（mealTypeId 形式）', function () {
    $user = $this->testData->createUserWithGroup();
    $recipe = $this->testData->createRecipeViaApi($this, $user);
    $courseType = $this->testData->createCourseTypeViaApi($user);
...
test('3-6-49: 【更新】 バリデーションエラー（mealTypeId 存在チェック）', function () {
    $user = $this->testData->createUserWithGroup();
    $recipe = $this->testData->createRecipeViaApi($this, $user);
    $courseType = $this->testData->createCourseTypeViaApi($user);
    ...
    $requestData = [
        'date' => '2024-01-16',
        'mealTypeId' => '12345678-1234-1234-1234-123456789012', // 存在しないmealTypeId
        'menu' => [
            [
                'recipeIds' => [$recipe['id']],
                'courseTypeId' => $courseType['id']
...
test('3-6-59: 【更新】 バリデーションエラー（courseTypeId 必須）', function () {
    $user = $this->testData->createUserWithGroup();
    $mealType = $this->testData->createMealTypeViaApi($this, $user);
    $recipe = $this->testData->createRecipeViaApi($this, $user);
...
test('3-6-60: 【更新】 バリデーションエラー（courseTypeId 形式）', function () {
    $user = $this->testData->createUserWithGroup();
    $mealType = $this->testData->createMealTypeViaApi($this, $user);
    $recipe = $this->testData->createRecipeViaApi($this, $user);
...
test('3-6-61: 【更新】 バリデーションエラー（courseTypeId 存在チェック）', function () {
    $user = $this->testData->createUserWithGroup();
    $mealType = $this->testData->createMealTypeViaApi($this, $user);
    $recipe = $this->testData->createRecipeViaApi($this, $user);
    ...
    $requestData = [
        ...
        'menu' => [
            [
                'recipeIds' => [$recipe['id']],
                'courseTypeId' => '12345678-1234-1234-1234-123456789012' // 存在しないcourseTypeId
            ]
        ]
    ];
