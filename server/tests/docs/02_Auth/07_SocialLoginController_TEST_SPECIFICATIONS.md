...
| 2-7-9  | 【コールバック】 新規ユーザー作成時に Google アバターを取り込み avatar_image_id を設定 | 正常系 | 新規ユーザーかつ getAvatar() が有効な URL         | `images` に保存し `users.avatar_image_id` が設定される、セッションログイン、/plan へ 302 | `SocialLoginController::handleGoogleCallback()` |
| 2-7-10 | 【コールバック】 Google CDN のアバター URL は高解像度（=s512-c）に正規化されてダウンロードされる | 正常系 | `=s96-c` などのサイズ指定付き Google CDN URL      | `=s512-c` に置換した URL でダウンロードされ、`users.avatar_image_id` が設定される        | `SocialLoginController::handleGoogleCallback()` |
