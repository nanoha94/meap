...
| 1-3-4 | 【handleException】 QueryException 処理テスト                            | 例外処理 | GET `/api/users`<br>QueryException           | - HTTP 500<br>- データベースエラーメッセージ<br>- ログに `sqlstate` / `error_code` のみ（SQL・バインド値・`getMessage()` は非露出） | `ExceptionHandlerTrait::handleException()` |
