...
| 3-1-28 | 【一括削除】 ImageService の deleteImages 内で想定外エラー                | 異常系 | ImageService::deleteImages 内で想定外エラー（本APIは紐づけ解除のみでファイル削除を行わない） | HTTP 500 Internal Server Error                                      | `ImageController::bulkDestroy()`   |
